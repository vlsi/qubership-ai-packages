...
- **Чекбокс, тоггл, радиокнопка.** Подпись — утверждение в несовершенном виде,
  без `?` в конце: «Сохранять ответ как хеш MD5», «Использовать бинарный режим».
  Английский паттерн `Save response as MD5 hash?` переводим утверждением, не
  вопросом. `?` оставляем только у диалогов подтверждения и пунктов меню-вопросов
  типа «Что это за узел?».
